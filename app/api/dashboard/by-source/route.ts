...
export async function GET(request: NextRequest) {
  ...
  const bySource: Record<string, { sourceName: string; scope: number; total: number; activityTotal: number; activityUnit: string }> = {};
  for (const e of emissions) {
    if (!bySource[e.sourceName]) {
      bySource[e.sourceName] = { sourceName: e.sourceName, scope: e.scope, total: 0, activityTotal: 0, activityUnit: e.activityUnit };
    }
    bySource[e.sourceName].total += e.totalEmissions;
    bySource[e.sourceName].activityTotal += e.activityData;
  }
